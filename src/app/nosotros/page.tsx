    ...
    >
      ...
      {i18n.CONTENT.map((sec) => (
        <section key={sec.TITLE}>
          <h2 className="sm:text-3xl text-2xl text-balance text-primary font-bold mt-8 mb-4">
            {sec.TITLE}
          </h2>
          ...
          {sec.PARAGRAPHS.map((parag, paragIndex) => {
            ...
            if (isList) {
              return (
                <ul className="list-disc ml-6" key={parag}>
                  <li className="py-2">{parag.replace("-", "")}</li>
                </ul>
              );
            }
            return (
